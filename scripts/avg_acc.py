...
def get_results(model_name, data_name):
    if model_name == "Meta-Llama-3-8B":
        ...
        use_context_files = [
            f"Llama-3.2-1B-{data_name}-10-0.07-2.0-grouped_activations_3shot_seeds42-43-multiprocess-mutual_information-grouped_activations_3shot_seeds42-43-False-True-42",
        ]
        use_parameter_files= []
    else:
        raise ValueError
    ...
    for cur_result in use_context_results:
        use_context_em.append(sum(cur_result["use_context_sub_scores"]) / len(cur_result["use_context_sub_scores"]) * 100)
    use_parameter_em = []
    for cur_result in use_parameter_results:
        use_parameter_em.append(sum(cur_result["use_parameter_org_scores"]) / len(cur_result["use_parameter_org_scores"]) * 100)
